...
5. **File uploads** → `lib/azure-blob-service.ts` uploads to Azure Blob Storage (`carespacbugattach` storage account, `bug-attachments` container)
...
3. `uploadFilesToAzureBlob()` uploads to Azure Blob Storage at `bug-attachments/{timestamp}-{filename}`
4. Returns public Azure Blob URLs (`https://carespacbugattach.blob.core.windows.net/bug-attachments/...`) for each file
5. URLs embedded in GitHub issue body as markdown links
6. Files sent to ClickUp as actual attachments (different API)

**Note**: Previously used GitHub Contents API (`uploadFilesToGitHub`), which triggered Vercel deployments on every upload. Migrated to Azure Blob Storage to eliminate unnecessary deployments.
...
# Azure Blob Storage (for bug report attachments)
AZURE_STORAGE_CONNECTION_STRING=DefaultEndpointsProtocol=https;AccountName=carespacbugattach;...
AZURE_BLOB_CONTAINER_NAME=bug-attachments
